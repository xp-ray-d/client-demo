...
import com.xp.client.dto.v2.IApi;
 ...
 * @Description: API通知请求报文
 * @Version: 1.0
 */
@Slf4j
@Data
@NoArgsConstructor
public class ApiNotifyReq implements IApi {
    ...
    public void check(final String origMsg) {
        if (StringUtils.isBlank(this.getBody())) {
            throw new BaseException(RspCodeStdEnum.ERROR_MSG_FORMAT, "通知报文[body]为空：[" + origMsg + "]");
        } else if (StringUtils.isBlank(this.getSign())) {
            throw new BaseException(RspCodeStdEnum.ERROR_MSG_FORMAT, "通知报文[sign]为空：[" + origMsg + "]");
        }
    }
}
